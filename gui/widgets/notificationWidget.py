from PyQt5.QtWidgets import QApplication, QLabel, QHBoxLayout, QWidget, QListWidget, QGridLayout, QSpacerItem, \
...
"""参考地址：https://github.com/PyQt5/PyQt"""
...
class NotificationItem(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.fillPath(path, QColor(255, 152, 0, 255))


class NotificationWidget(QListWidget):
    _instance = None
    ...
    def _createInstance(cls):
        ...
        if not cls._instance:
            cls._instance = NotificationWidget()
            cls._instance.show()
            NotificationIcon.init()

    @classmethod
    def info(cls, title, message, callback=None, pos=None):
        cls._createInstance()
        ...
        if pos != None:
            cls._instance.move(pos[0], pos[1])

    @classmethod
    def success(cls, title, message, callback=None, pos=None):
        cls._createInstance()
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, cls._instance,
    ...
    @classmethod
    def warning(cls, title, message, callback=None, pos=None):
        cls._createInstance()
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, cls._instance,
    ...
    @classmethod
    def error(cls, title, message, callback=None, pos=None):
        cls._createInstance()
        item = QListWidgetItem(cls._instance)
